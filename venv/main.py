...
# -----------------------------
# ORDER CREATION (REQUIRED FOR Q4 & BONUS)
# -----------------------------
@app.post("/orders")
...
# -----------------------------
@app.get("/sort-by-category")
def sort_by_category(category: str = Query(...)):
    if category not in ["Electronics", "Stationary"]:
        return {"error": "category must be 'Electronics' or 'Stationary'"}
    result = sorted(
        products,
